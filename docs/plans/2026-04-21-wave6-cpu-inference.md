...
**병렬 가능**: Task 4 (ORT static INT8) ∥ Task 5 (OpenVINO NNCF) — 서로 다른 프레임워크, 동일 `run_cpu.py` 파일이지만 함수 단위 분리.
...
- [ ] **Step 3: Extend `_schemas.py`**
  ...
  - `MeasurementSpec.thread_count: Optional[int] = None` (None이면 physical cores auto-detect)
...
- [ ] **Step 3: Implement skeleton**
  ...
  - `_resolve_weights(recipe)` 재사용 (OMNI_WEIGHTS_OVERRIDE 동일)
  - `_export_onnx(weights, imgsz, half=False)` — 기존 헬퍼 재사용 (dynamic=True)
  - FP32 경로: `ort.InferenceSession(onnx_path, providers=["CPUExecutionProvider"])`
  - session_options에 thread_count, graph_optimization_level=ORT_ENABLE_ALL 설정
...
- [ ] **Step 3b: 공통 calibration loader 추가 (Task 4 + Task 5 공유)**
  ...
  - COCO val 로딩 로직은 `run_trt.py`의 기존 헬퍼 재사용 (`_load_calib_images` 또는 동등)
...
- [ ] **Step 4: Implement `_prepare_openvino_model`**
  ...
  if recipe.runtime.dtype == "int8":
      import nncf
      calib_dataset = nncf.Dataset(calib_loader, transform_fn=lambda x: x[0])
  ...
  compiled = core.compile_model(ov_model, "CPU", {
      "PERFORMANCE_HINT": "LATENCY",  # bs=1 최적화
...
- [ ] **Step 3: Thread count 자동 감지**
  - recipe가 `thread_count=null`이면 `psutil.cpu_count(logical=False)` 또는 Linux `lscpu`에서 physical cores 감지
  - `session_options.intra_op_num_threads = thread_count`
  - Windows는 `os.cpu_count() // 2` 폴백 (hyperthreading 가정)
...
- [ ] **Step 1: `30_ort_cpu_fp32.yaml`**
  ...
  constraints:
    ...
    min_fps_bs1: 10  # CPU baseline이므로 GPU 대비 훨씬 낮게
  ```
- [ ] **Step 2: `31_ort_cpu_bf16.yaml`** — `dtype: bf16`, 하드웨어 요구 `requires_isa: ["avx512_bf16"]` 추가 (hardware gate 필드)
- [ ] **Step 3: `32_ort_cpu_int8_dynamic.yaml`** — `dtype: int8`, `calibrator: null`
- [ ] **Step 4: `33_ort_cpu_int8_static.yaml`** — `dtype: int8`, `calibrator: entropy`, `calibration_samples: 512`
- [ ] **Step 5: `34_openvino_fp32.yaml`** — `source: openvino`, `runtime.engine: openvino`
- [ ] **Step 6: `35_openvino_int8_nncf.yaml`** — `source: openvino`, `calibrator: nncf`, `calibration_samples: 512`
