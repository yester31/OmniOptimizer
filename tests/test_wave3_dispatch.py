"""Wave 3 schema + dispatch tests for ort_quant.

Scope:
- TechniqueSpec.source Literal accepts the supported backend names.
- _prepare_onnx dispatcher routes ort_quant to the right helper (mocked —
  actual quantization is exercised in end-to-end recipe runs).
"""
...
@pytest.mark.parametrize("src", ["trt_builtin", "modelopt", "ort_quant"])
