...
| 11 | TensorRT                   | INT8 PTQ + 2:4 Sparsity flag (Ampere+)           | `modelopt`   |
...
> **v1.2 참고 — 진짜 2:4 structured sparsity (post-training)**
>
> `technique.sparsity_preprocess: "2:4"` 옵션이 schema/runner에 구현돼 있고
> `modelopt.torch.sparsify(mode="sparse_magnitude")`를 제대로 호출하지만, nano YOLO
> (YOLO26n, ~2.4M params)에서는 fine-tuning 없는 post-training 2:4가 mAP를 사실상
> 0으로 무너뜨립니다. 이득은 sparsity-aware training (SAT) 또는 QAT가 동반될 때만
> 나오므로 v1.3으로 이관. 현재 레시피 #11은 dense 가중치 + `SPARSE_WEIGHTS` hint
> 조합으로 돌아갑니다 (no-op에 가까움, 수치는 recipe-09와 동등).
